...
public class BackendsXmlParser {
    ...
    public static BackendsXml parse(InputStream is)  {
        if (is == null) {
            return new BackendsXml();
        }

        BackendsXml backends = new BackendsXml();
        try {
            Document doc = XmlUtils.parseXml(is);

            if (BACKENDS_TAG.equals(doc.getDocumentElement().getTagName()) == false) {
                throw new CapedwarfConfigException("backends.xml does not contain <backends>");
            }
            ...
            throw new CapedwarfConfigException("Could not parse WEB-INF/backends.xml", e);
    ...
    private static Element getChildElement(Element parent, String childName, boolean allowNull) {
        ...
        if (allowNull) {
            ...
            throw new CapedwarfConfigException(parent.getTagName() + " does not contain <" + childName + ">");
        }
    }
}
